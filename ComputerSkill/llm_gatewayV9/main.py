...
# Load shared session-level env first, then allow llm_gatewayV9/.env to
# override it. The V9 README/run workflow keeps provider settings next to
# the gateway, and without the second load edits such as NVIDIA_MODEL changes
# silently keep using the parent .env value until the user exports vars by hand.
...
def _candidate_order(router: Router, provider: str | None, agent_pin: str | None) -> list[str]:
    if provider:
        return router.candidates(provider)
    candidates = list(router.order)
    if agent_pin and agent_pin in candidates:
        candidates = [agent_pin] + [c for c in candidates if c != agent_pin]
    return candidates
...
async def chat(req: ChatRequest):
    ...
    # as an ordering preference, not as a hard override. The prior
    # implementation mutated req.provider here, which made a flaky preferred
    # provider block failover entirely.
    ...
    if req.auto_route and not req.provider and not agent_pin:
        ...
        candidates = _candidate_order(router, req.provider, agent_pin)
...
async def list_providers():
    ...
    return {
        ...
        "vision_models": {n: p.vision_model for n, p in r.providers.items() if getattr(p, "vision_model", "")},
    }
...
async def capabilities():
    ...
    for name, p in r.providers.items():
        ...
        # per-model overrides
        caps = P.model_capabilities(name, p.model, caps, getattr(p, "vision_model", None))
        caps["model"] = p.model
        if getattr(p, "vision_model", ""):
            caps["vision_model"] = p.vision_model
        caps.update({
