...
class BrowserSkill:
    ...
    async def run(self, node: NodeSpec) -> AgentResult:
        if node.metadata.get("browser_extension") == "visible_catalog_report":
            from .visible_catalog_extension import run_visible_catalog_report
            return await run_visible_catalog_report(
                node,
        ...
        if not url:
            return self._with_replay_report(
                self._pack_error("", goal, "interaction_failed",
        ...
        if html:
            ...
            if block:
                return self._with_replay_report(
                    self._pack_error(url, goal, "gateway_blocked",
            ...
            if _is_useful_extract(content, goal):
                return self._with_replay_report(
                    self._pack(url, goal, "extract", turns=0,
        ...
        if selectors:
            ...
            if det is not None:
                return self._with_replay_report(
                    det if det.success else self._pack_error(
        ...
        if a11y_result.success:
            return self._with_replay_report(
                self._pack_driver("a11y", url, goal, a11y_result,
        ...
        if vis_result.success:
            return self._with_replay_report(
                self._pack_driver("vision", url, goal, vis_result,
        ...
        return self._with_replay_report(
            self._pack_error(url, goal, "interaction_failed",
    ...
    def _with_replay_report(self, result: AgentResult, node: NodeSpec,
                            started_at: float) -> AgentResult:
