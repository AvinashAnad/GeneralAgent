...
OLLAMA_THINKING_MODELS = ("qwen", "deepseek-r1", "gpt-oss")
...
def _ollama_disable_thinking(model: str) -> bool:
    setting = os.getenv("OLLAMA_THINK", "false").strip().lower()
    if setting in {"1", "true", "yes", "on"}:
        return False
    return any(h in (model or "").lower() for h in OLLAMA_THINKING_MODELS)
...
class OllamaProvider(BaseProvider):
    ...
    def __init__(self, model, base_url="http://localhost:11434", vision_model: str | None = None):
        super().__init__("", model, base_url)
        self.vision_model = vision_model or ""

    def _select_model(self, messages, model: str | None = None) -> str:
        if model:
            return model
        if self.vision_model and any(_content_has_image(m.get("content")) for m in messages):
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
                   tools=None, tool_choice=None, reasoning=None, response_format=None,
                   system_blocks=None, cache_system=False):
        m = self._select_model(messages, model)
        system_text, _, _ = _flatten_system(system_blocks)
...
def model_capabilities(provider_name: str, model: str, default_caps: dict, vision_model: str | None = None) -> dict:
    ...
    caps["vision"] = bool(vision_model) if provider_name == "ollama" else _model_supports_vision(provider_name, model)
    return caps
...
def build_providers(cache_store):
    ...
    if k := os.getenv("NVIDIA_API_KEY"):
        out["nvidia"] = NvidiaProvider(k, os.getenv("NVIDIA_MODEL", "minimaxai/minimax-m3"))
    ...
    for name, p in out.items():
        p.capabilities = model_capabilities(
            name,
            p.model,
            getattr(p, "capabilities", {}),
            getattr(p, "vision_model", None),
        )
    return out
