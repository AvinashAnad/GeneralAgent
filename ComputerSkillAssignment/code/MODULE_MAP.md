# S9SharedCode Module Map

This document maps the `S9SharedCode/code` runtime: what each module owns,
...
| `BrowserOutput` | Browser result payload. | `url`, `goal`, `path`, `turns`, optional `content`, `actions`, optional `final_url`. | `browser.skill`. |
