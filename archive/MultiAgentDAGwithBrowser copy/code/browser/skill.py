...
Assignment 9 extends the shipped skill with browser-session metadata:
`browser_profile`, `storage_state`, `save_storage_state`, `user_data_dir`,
and `headless`. The cascade stays the same; the browser layers simply
receive the right Playwright context when a target needs cookies or
localStorage.
"""
...
from .session import BrowserSessionConfig
...
_GATEWAY_BLOCK_MARKERS = (
    ...
    # Geo/rate/WAF blocks. These are still precondition failures: the page
    # did not make the requested content available to the browser session.
    ("geo_block",              "not available in your region"),
    ("geo_block",              "not available in your country"),
    ("rate_limited",           "too many requests"),
    ("rate_limited",           "rate limit exceeded"),
    ("rate_limited",           "unusual traffic from your computer network"),
...
class BrowserSkill:
    ...
    async def run(self, node: NodeSpec) -> AgentResult:
        ...
        session_cfg = BrowserSessionConfig.from_metadata(node.metadata)
        session_meta = session_cfg.output_metadata()
        t0 = time.time()
        ...
        if selectors:
            det = await self._try_deterministic(url, goal, selectors, session_cfg)
            if det is not None:
        ...
        if force_path == "vision":
            ...
            a11y_result = await self._drive(
                ...
                self.a11y_provider_pin, self.max_steps_a11y, session_cfg,
            )
        if getattr(a11y_result, "gateway_blocked", False):
            return self._pack_error(url, goal, "gateway_blocked",
                                    a11y_result.note or "gateway_blocked after JS render",
                                    elapsed=time.time() - t0,
                                    block_type=getattr(a11y_result, "gateway_block_type", None),
                                    session_meta=session_meta)
        ...
        vis_result = await self._drive(
            ...
            self.vision_provider_pin, self.max_steps_vision, session_cfg,
        )
        if getattr(vis_result, "gateway_blocked", False):
            return self._pack_error(url, goal, "gateway_blocked",
                                    vis_result.note or "gateway_blocked after JS render",
                                    elapsed=time.time() - t0,
                                    block_type=getattr(vis_result, "gateway_block_type", None),
                                    session_meta=session_meta)
    ...
    async def _drive(self, DriverCls, url, goal, client, artifacts_dir,
                     provider_pin, max_steps, session_cfg: BrowserSessionConfig):
        ...
        async with async_playwright() as p:
            browser, ctx = await self._open_context(p, session_cfg)
            ...
            try:
                ...
                await self._close_context(browser, ctx, session_cfg)
    ...
    async def _try_deterministic(
        ...
        goal,
        selectors,
        session_cfg: BrowserSessionConfig,
    ) -> AgentResult | None:
    ...
    async def _open_context(self, playwright, session_cfg: BrowserSessionConfig):
        if session_cfg.user_data_dir:
            session_cfg.user_data_dir.mkdir(parents=True, exist_ok=True)
            ctx = await playwright.chromium.launch_persistent_context(
                str(session_cfg.user_data_dir),
                headless=session_cfg.headless,
                args=_CHROMIUM_ARGS,
                **_BROWSER_CONTEXT_OPTIONS,
            )
        ...
        ctx = await browser.new_context(
            **session_cfg.new_context_options(**_BROWSER_CONTEXT_OPTIONS)
        )
        await ctx.add_init_script(_STEALTH_SCRIPT)
        return browser, ctx

    async def _close_context(self, browser, ctx, session_cfg: BrowserSessionConfig) -> None:
        try:
            await session_cfg.save_context(ctx)
        finally:
            if browser is None:
                await ctx.close()
            else:
    ...
    def _pack(self, url, goal, path, *, turns, content=None, actions=None,
              final_url=None, elapsed=0.0, session_meta=None,
              block_type=None) -> AgentResult:
        ...
        out = BrowserOutput(
            ...
            storage_state_path=session_meta.get("storage_state_path"),
            user_data_dir=session_meta.get("user_data_dir"),
    ...
    def _pack_driver(self, path, url, goal, drv_result,
                     *, final_url, elapsed, session_meta=None) -> AgentResult:
        session_meta = session_meta or {}
        out = BrowserOutput(
    ...
    def _pack_error(self, url, goal, code, msg, *, elapsed=0.0,
                    block_type=None, session_meta=None) -> AgentResult:
        session_meta = session_meta or {}
        out = BrowserOutput(
