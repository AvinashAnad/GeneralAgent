    ...
    async def run(self, query: str, *, session_id: str | None = None,
        ...
        while True:
            ...
            for nid, result, prompt in outcomes:
                ...
                if result.success and graph.g.nodes[nid]["skill"] == "critic":
                    verdict = (result.output or {}).get("verdict", "pass")
                    rationale = (result.output or {}).get("rationale", "")
                    if rationale:
                        print(f"  ↪ critic verdict={verdict}: {rationale[:240]}")
                    else:
