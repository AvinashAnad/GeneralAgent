...
from provider_routing import chat_with_provider_order
...
class Skill:
    def __init__(self, name: str, cfg: dict):
        ...
        self.provider_order: list[str] = list(cfg.get("provider_order") or [])
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if tools:
        ...
        if skill.provider_order:
            reply = await asyncio.to_thread(
                chat_with_provider_order,
                provider_order=skill.provider_order,
                prompt=rendered,
                agent=skill.name,
                session=session_id,
                max_tokens=skill.max_tokens,
