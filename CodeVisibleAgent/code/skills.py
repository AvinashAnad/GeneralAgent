...
def _looks_like_desktop_query(query: str) -> bool:
    q = (query or "").lower()
    desktop_terms = (
        "local app", "desktop", "calculator", "finder", "file explorer",
        "system settings", "settings app", "installed app", "electron",
        "window", "screen", "ax tree", "accessibility tree", "cua-driver",
    )
    if any(term in q for term in desktop_terms):
        return True
    return (" app " in f" {q} " or "application" in q) and any(v in q for v in ("open", "use", "click", "type", "verify"))


def _desktop_fallback_plan(query: str) -> dict:
    q = (query or "").lower()
    computer_md = {
        "label": "desktop",
        "goal": query,
    ...
    if "calculator" in q:
        computer_md.update({
            ...
            "perception_query": "button",
        })
    return {
        "rationale": "The request targets a local desktop app, so run Computer and then format the verified result.",
        "nodes": [
            {"skill": "computer", "inputs": [], "metadata": computer_md},
            {"skill": "formatter", "inputs": ["USER_QUERY", "n:desktop"], "metadata": {"label": "out"}},
        ],
    }
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "session_reporter":
        ...
        if analysis.get("markdown"):
            print(f"[session_reporter] analysis: {analysis['markdown']}")
        return result, rendered
    ...
    if skill.name == "planner" and not parsed.get("nodes") and not parsed.get("successors"):
        if _looks_like_desktop_query(query):
            parsed = _desktop_fallback_plan(query)
        else:
            err = "planner: malformed or empty plan; no nodes emitted"
            return AgentResult(
                success=False,
