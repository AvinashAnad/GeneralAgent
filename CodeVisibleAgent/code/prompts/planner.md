...
Available skills:
  ...
  browser            fetch / interact with a SPECIFIC URL through a
                     ...
                     when needed.
  computer           drive an installed LOCAL DESKTOP APP through
                     cua-driver. Use this for local/Mac/Windows/Linux
                     apps, native dialogs, Calculator, Finder/File
                     Explorer, System Settings, Electron apps, AX
                     trees, desktop screenshots, record trajectory,
                     scan-act-verify loops, and desktop UI automation.
                     metadata MUST set goal (str). Prefer app and
                     bundle_id when known. Useful metadata:
                       app, bundle_id, window_title, max_steps,
                       perception_query, allow_vision.
                     The skill records native cua-driver trajectories
                     plus a semantic ComputerOutput.actions ledger.
...
structured records the Formatter can render cleanly.
  ...
  coder              emit Python (stub; routes to sandbox_executor).
                     Use only for code generation or pure sandboxed
                     computation. Do not use coder/sandbox_executor
                     to operate local desktop apps, GUI apps, OS
                     settings, Calculator, AX trees, or screen state.
...
Desktop routing hard rules:
  - If the query asks to use/open/control a local app or desktop UI,
    emit a `computer` node followed by `formatter`.
  - The formatter should receive USER_QUERY plus the computer node so
    it can preserve strict final-answer constraints.
  - On recovery, if a previous `computer` node already completed, do
    not re-run it just to format/report the result; wire the completed
    `n:*` output directly to `formatter`.

Example — local desktop app:
{"rationale": "The task requires operating a local desktop app, so use Computer and then format the verified result.",
 "nodes": [
   {"skill":"computer","inputs":[],
    "metadata":{"label":"desktop","goal":"Use the requested local desktop app to complete the task and verify the final state.","app":"<app if known>","bundle_id":"<bundle id if known>","max_steps":12,"allow_vision":true}},
   {"skill":"formatter","inputs":["USER_QUERY","n:desktop"],
    "metadata":{"label":"out"}}]}
...
on the same inputs. In particular: if FAILURE is for a Browser node
or mentions `gateway_blocked`, `interaction_failed`, `all layers
exhausted`, `step cap reached`, or `giveup`, treat that site/browser
attempt as exhausted. Do NOT retry Browser for the same task, even
with a different URL on the same retailer. Recover by using
`researcher` for open web research, by using `retriever` only when
memory hits clearly contain the answer, or by handing back to the
user with the formatter.
